...
proto.getUpdateCheckCacheKey = function (
  deploymentKey,
  appVersion,
  label,
  packageHash
...
proto.clearUpdateCheckCache = function (
  deploymentKey,
  appVersion,
  label,
  packageHash
...
proto.chosenMan = function (packageId, rollout, clientUniqueId) {
  ...
  var rolloutClientUniqueIdCache = _.get(
    config,
  ...
  if (rolloutClientUniqueIdCache === false) {
    ...
    var client = factory.getRedisClient("default");
    var redisCacheKey = self.getChosenManCacheKey(
      packageId,
      rollout,
      clientUniqueId
    );
...
proto.updateCheck = function (
  ...
  if (_.isEmpty(deploymentKey) || _.isEmpty(appVersion)) {
    return Promise.reject(
      new AppError.AppError("please input deploymentKey and appVersion")
    );
  }
...
proto.getPackagesInfo = function (deploymentKey, label) {
  if (_.isEmpty(deploymentKey) || _.isEmpty(label)) {
    return Promise.reject(
      new AppError.AppError("please input deploymentKey and label")
    );
  }
  return models.Deployments.findOne({
    where: { deployment_key: deploymentKey },
    ...
    .then((dep) => {
      ...
      return models.Packages.findOne({
        where: { deployment_id: dep.id, label: label },
      });
    })
    .then((packages) => {
...
proto.reportStatusDownload = function (deploymentKey, label, clientUniqueId) {
  return this.getPackagesInfo(deploymentKey, label).then((packages) => {
    return Promise.all([
      models.PackagesMetrics.findOne({
...
proto.reportStatusDeploy = function (
  deploymentKey,
  label,
  clientUniqueId,
  others
