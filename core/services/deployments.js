...
proto.addDeloyment = function (name, appId, uid) {
  var self = this;
  return models.Users.findById(uid).then((user) => {
    if (_.isEmpty(user)) {
    ...
    return models.Deployments.update(
      ...
      if (_.gt(affectedCount, 0)) {
        ...
        throw new AppError.AppError(
          `does not find the deployment "${deploymentName}"`
        );
      }
    });
  });
...
proto.findPackagesAndOtherInfos = function (packageId) {
  return models.Packages.findOne({
    ...
    return Promise.props({
      ...
      deploymentsVersions: models.DeploymentsVersions.findById(
...
proto.findDeloymentsPackages = function (deploymentsVersionsId) {
  ...
  return models.DeploymentsVersions.findOne({
    ...
    if (deploymentsVersionsInfo) {
      return self.findPackagesAndOtherInfos(
...
proto.formatPackage = function (packageVersion) {
  ...
  return {
    ...
    isDisabled: false,
    isMandatory:
      _.get(packageVersion, "packageInfo.is_mandatory") == 1 ? true : false,
    rollout: 100,
    ...
    uploadTime: parseInt(
      moment(_.get(packageVersion, "packageInfo.updated_at")).format("x")
    ),
    originalLabel: _.get(packageVersion, "packageInfo.original_label"),
    originalDeployment: _.get(
      packageVersion,
      "packageInfo.original_deployment"
    ),
...
proto.listDeloyments = function (appId) {
  var self = this;
  return models.Deployments.findAll({ where: { appid: appId } }).then(
    (deploymentsInfos) => {
