...
proto.findPackageInfoByDeploymentIdAndLabel = function (deploymentId, label) {
  return models.Packages.findOne({
    where: { deployment_id: deploymentId, label: label },
  });
};
...
proto.downloadPackageAndExtract = function (
  workDirectoryPath,
  packageHash,
  blobHash
) {
  ...
  return models.PackagesDiff.findOne({
    ...
    return common
      ...
      .then(() => {
        ...
        var json = common.diffCollectionsSync(
          originManifestJson,
          diffManifestJson
        );
        ...
        if (isUseDiffText == constConfig.IS_USE_DIFF_TEXT_YES) {
          ...
          _.forEach(json.diff, function (tmpFilePath) {
            ...
            if (
              ...
              if (
                patchText &&
                patchText.length < _.parseInt(textNew.length * 0.8)
              ) {
...
proto.createDiffPackages = function (
  originalPackage,
  destPackages,
  isUseDiffText
) {
  ...
  var workDirectoryPath = path.join(
    os.tmpdir(),
    "codepush_" + security.randToken(32)
  );
  log.debug("workDirectoryPath", workDirectoryPath);
  return common
    .createEmptyFolder(workDirectoryPath)
    .then(() =>
      self.downloadPackageAndExtract(workDirectoryPath, package_hash, blob_url)
    )
...
proto.releasePackage = function (
  ...
  var directoryPathParent = path.join(
    tmpDir,
    "codepuh_" + security.randToken(32)
  );
    ...
    .then((packageInfo) => {
      ...
      if (!_.isNull(appVersion)) {
        ...
        return Promise.all([
          ...
          models.DeploymentsVersions.findById(
      ...
      return models.Packages.findOne({
        ...
        .then((sourcePack) => {
          ...
          return models.DeploymentsVersions.findById(
            ...
            if (!deploymentsVersions) {
              throw new AppError.AppError(
                "deploymentsVersions does not exist."
              );
            }
...
proto.rollbackPackage = function (
  deploymentVersionId,
  targetLabel,
  rollbackUid
) {
  var self = this;
  return models.DeploymentsVersions.findById(deploymentVersionId).then(
    (deploymentsVersions) => {
      if (!deploymentsVersions) {
